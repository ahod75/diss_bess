...
if __name__ == "__main__":
    ...
    # --- reindex to complete hourly grid + IMPUTE --------------------------------
    ...
    # --- windowing: full splits for baseline forecast ---------------------------- (cell 7) ---
    h = pd.Timedelta(hours=1)
    Xtr = make_windows(frame_full,
                       y_range=(TRAIN_START, VAL_START - h),
                       gate_aligned_only=False,
                       issue_hour = 9,
                       exo_cols=EXO_COLS,
                       hist_cols=HIST_COLS,
    ...
    # --- Test 1: month split (the experiment wired to training) --- (cells 11,13)-
    ...
    print(
        "shared scaler check  mu_y == mu_hist[0]:",
        np.isclose(sc["mu_y"], sc["mu_hist"][0]),
        "| sd_y == sd_hist[0]:",
        np.isclose(sc["sd_y"], sc["sd_hist"][0]),
    )
    ...
    model = Baseline_Forecaster(
        n_hist_features=num_hist,
        n_exo_features=num_exo,
        horizon=24,
        n_quantiles=len(QUANTILE_LEVELS),
        gru_hidden_size=32,
        n_gru_layers=2,
        dense_hidden_width=64,
        dropout=0.1,
        eps=1e-4,
    )
    ...
    for epoch in range(30):
        ...
        for i in range(0, len(xh_n), 64):
            b = perm[i : i + 64]
            opt.zero_grad()
            loss = pinball_loss(
                y_n_t[b], model(xh_n_t[b], x_fut_tr_t[b]), QUANTILE_LEVELS_TENSOR
            )
            loss.backward()
            opt.step()
        if (epoch + 1) % 30 == 0:
            ...
            with torch.no_grad():
                vl = pinball_loss(
                    y_val_n_t, model(xh_val_n_t, x_fut_val_t), QUANTILE_LEVELS_TENSOR
                ).item()
    ...
    model.eval()
    with torch.no_grad():
        q_va = model(xh_val_n_t, x_fut_val_t)  # (N, H, Q), standardised
    ...
    with torch.no_grad():
        cover = [
            (y_val_n_t.unsqueeze(-1) <= q_va[..., k : k + 1]).float().mean().item()
    ...
    # --- evaluate: raw validation prosumption ----------------------- (cell 25) --
