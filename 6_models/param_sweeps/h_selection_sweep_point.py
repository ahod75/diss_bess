...
Ported from models_robust/h_sweep_old/h_selection_sweep_point.py to the modular
dispatch pipeline (dispatch_setup.setup_point_robust / dispatch_shared.py /
dispatch_wrapper.py) instead of dispatch_layer_point_robust.py / dispatch_wrapper_robust.py.

Identical methodology to h_selection_sweep.py (see that file's docstring for the full
rationale), except it solves the point-robust formulation instead of the full LP-dual
robust counterpart -- i.e., it measures what h_plus/h_minus actually do to the model
that will actually be TRAINED against, rather than the exact-worst-case formulation used
only for final testing. Run both and compare: if the elbows land in the same place, one
box suffices for both training and testing; if they diverge, the surrogate's weaker
(2-point, not exact-worst-case) feasibility guarantee needs its own calibration.

p_da_rel is reconstructed manually (p_ch_hat - p_dis_hat) since it's a plain expression
in the point-robust formulation, not one of the layer's output variables (unlike
full-robust, where it IS a layer output -- see dispatch_setup.py's setup_point_robust
vs setup_full_robust for the reason).
"""
...
def run_sweep(windows, forecast_fn, sampler, quantile_levels):
    ...
    for d in range(n_days):
        ...
        for pm, k in CORNERS:
            fp = fps[k]
            bundle = bundles[(pm, k)]
            prices = get_prices(price_day, pm, cols=PRICE_COLS)
            base_vals = {"Sigma_xi_chol": Sigma_np, "xi_samples": xi_np, **prices}
...
if __name__ == "__main__":
    ...
    per_day_path = PARAM_SWEEPS_DIR / "h_sweep_per_day_2018_point.csv"
    yearly_path = PARAM_SWEEPS_DIR / "h_sweep_yearly_2018_point.csv"
