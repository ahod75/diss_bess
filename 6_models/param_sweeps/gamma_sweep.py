"""gamma_sweep.py -- profiles EVERY differentiable surrogate corner's (point_robust +
1stage) forward and backward solve across Tikhonov `gamma` values, on a fixed 16-day
batch drawn from the training year.

Expanded from the original dual-price-only version, which swept only point_robust
dual-price on the reasoning "worst-conditioned corner, so a safe gamma there is safe
everywhere". Now that gamma is being justified as a single, uniform, cross-corner choice
(applied identically to every architecture/mode rather than tuned per corner -- see
default GAMMA in dfl_train_utils.py, used unmodified by every corner in
train_dfl_models.py and every call site in evaluate.py), this sweep measures ALL
differentiable corners directly instead of extrapolating from one.

Only differentiable (cvxpylayers-backed) corners are covered here -- point_robust and
1stage. The full LP-dual robust counterpart's backward pass is not attempted (established
early in this project as too computationally intense to run at all -- see this branch's
first commit); its own gamma sensitivity is covered separately, FORWARD-ONLY, by
...
solve pipeline, unlike the original version of this script) -- guarantees the sweep tests
exactly what training actually does, no drift risk. Trade-off vs. the original: no
retain_graph=True sharing of the forecast pass across gamma values, so each gamma value
re-runs its own forecast -- negligible cost next to the QP solve, and far simpler/safer
than threading a shared graph through five different (architecture, mode) problem shapes.
...
WARNING: batch_size=16 is double the live batch_size=8 -- check `free -h` before running
this alongside a live training run.
...
def precompute_boxes(train_windows, batch_indices, baseline_model, sc, sampler, quantile_levels):
    """point_robust corners only -- ignored (harmlessly) when architecture=='1stage', since
    dfl_loss_batch only reads `boxes` inside its `if architecture == "point_robust":` branch."""
    from dfl_train_utils import compute_box, BOX_LEVELS
    boxes = {}
    with torch.no_grad():
...
def run_one(architecture, mode, gamma, batch_indices, model, sampler, sc, train_windows, fwd, boxes):
...
if __name__ == "__main__":
    ...
    print("precomputing boxes for the 16 sampled days (point_robust corners)...")
    boxes = precompute_boxes(train_windows, batch_indices, baseline_model, sc, sampler, cop["quantile_levels"])

    results = []
    for architecture, mode in CORNERS:
        print(f"\n--- corner: architecture={architecture} mode={mode} ---", flush=True)
        for gamma in GAMMAS:
            r = run_one(architecture, mode, gamma, batch_indices, model, sampler, sc, train_windows, fwd, boxes)
            results.append(r)
