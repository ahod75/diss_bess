...
# =====================================================================================
# dispatch_layer.py  --  FREE-BID robust LDR dispatch, single- & dual-price objectives.
#
...
# (variables, SOC, robust box, tracking, penalty, bid bounds) is shared verbatim.
#
# KEY DIFFERENCE FROM THE OLD (tied) REFERENCE CODE:
#   The bid p^{da} is now a FREE variable (Stratigakos-style), reparametrised as
#       p_da_rel = p^{da} - pl_hat           (variable-only, keeps sum_squares(imb_det) DPP)
#   so the deterministic imbalance offset is
#       imb_det = (p_hat_ch - p_hat_dis - p_da_rel)          [ = g_hat - p^{da} ]
#   and the realised imbalance is
#       p_imb(xi) = imb_det + R xi ,   R = I + D_ch - D_dis .
...
def default_fixed_params(k: float, num_scenarios: int = 64) -> FixedParams:
    ...
    return FixedParams(
        ...
        p_min=-5.0, p_max=11.0, k=float(k), gamma=1e-6,
    )
...
def build_problem(fp: FixedParams, price_model: str) -> ProblemBundle:
    ...
    if fp.k < 1.0:
        ...
        if price_model == "single":
            ...
            # C_imb = sum(lam_up @ cp.pos(p_imb) + lam_down @ cp.neg(p_imb)) * dt / N
            ...
            ones_N = np.ones((1, N))
            imb_scen = cp.reshape(imb_det, (T, 1)) @ ones_N + R @ xi_samples.T   # (T, N)
            cons += [p_plus - p_minus == imb_scen]
            # math (.)^- <= 0  maps to code p_minus >= 0 :  -lam_dn (.)^- = lam_dn . p_minus
            C_imb = cp.sum(lam_up @ p_plus + lam_dn @ p_minus) * dt / N
...
def build_oracle(fp: FixedParams, price_model: str) -> OracleBundle:
    assert price_model in ("single", "dual")
    T = fp.T_total
    ...
    if price_model == "single":
        ...
        lam_up = cp.Parameter(T, nonneg=True, name="lam_up")
        lam_dn = cp.Parameter(T, nonneg=True, name="lam_dn")
        param_by_name["lam_up"], param_by_name["lam_dn"] = lam_up, lam_dn
        # plain solve -> DPP not required -> cp.pos/cp.neg is fine and clean.
        C_imb = (lam_up @ cp.pos(imb) + lam_dn @ cp.neg(imb)) * fp.dt
