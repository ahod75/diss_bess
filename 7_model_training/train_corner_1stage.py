...
from dispatch_layer_1stage import default_fixed_params, build_problem, make_layer
from dispatch_layer import build_oracle, solve_oracle   # oracle is k/D_ch-independent, reuse as-is
from dispatch_wrapper import (get_prices, oracle_price_values, realised_cost,
                              realised_breakdown, per_day_metrics)
...
def solve_with_retry(layer, args):
    ...
    except cvxpylayers_solver_error() as e:
...
def dfl_loss_batch(batch_indices, *, model, fp, sampler, sc, windows, layer, keys, price_model,
    ...
    vals = build_layer_vals(mean, prices, device)
    ...
    try:
        ...
        per_day, L_base, f_dfl, sat = _combine(dec, realised, mean, prices, q_norm_batch,
    ...
    total = 0.0; n_survived = 0; L_base_total = 0.0; f_dfl_total = 0.0
    for i in range(B):
    ...
    return total, n_survived, 0, n_survived * T, L_base_total, f_dfl_total
...
def evaluate_regret(*, model, fp, sampler, sc, windows, oracle_costs, layer, keys, price_model,
    ...
    with torch.no_grad():
        for d in range(len(windows.delivery_start)):
            try:
                combined, n_survived, _, _, L_base_sum, f_dfl_sum = dfl_loss_batch(
