"""train_dfl_archetypes.py -- trains 1stage single-price/dual-price forecasters under
...
buffer) and long_slow (low power, high energy -- slow reaction, lots of buffer).

The "balanced" archetype (C_ch=C_dis=2.0, B_max=4.0, duration=2h) is the SAME physical
configuration train_dfl_models.py already trains (dfl_1stage_{mode}.pt) -- not retrained
here, reused directly as the third grid point when comparing archetypes.
...
the broader pivot toward 1stage-only training going forward (point_robust's own
retraining need is being handled separately, via the gamma/h-sweep-driven final retrain).

Mirrors train_dfl_models.py's structure (load data once, per-corner try/except, gc.collect
after every corner, forward/backward timing) but skips box precomputation entirely --
1stage has no h_plus/h_minus Parameters to feed regardless of architecture, so
train_dfl_models.py's `boxes_train`/`boxes_val` machinery is dead weight here.

Usage: python train_dfl_archetypes.py
"""
...
# (name, C_ch, C_dis, B_max) -- SOC0 derived as 0.5*B_max, matching the "balanced"
# default's 2.0/4.0 convention. "balanced" itself (2.0, 2.0, 4.0) is NOT listed here --
# already trained by train_dfl_models.py as dfl_1stage_{mode}.pt.
ARCHETYPES = [
    ...
    for arch_name, C_ch, C_dis, B_max in ARCHETYPES:
        for mode in MODES:
            ...
            try:
                ...
                out_path = DFL_TRAIN_DIR / f"dfl_1stage_{mode}_{arch_name}.pt"
                ...
                trained_model, best_val, hist = train_one_config(
                    ...
                    boxes_train=None, boxes_val=None, device=device, fwd=fwd, out_path=out_path)
